...
@Injectable()
export class ClasProfesional2PrService {
  public datos;
  config: any;
 ...
 getDatosModelo() {
    ...
    let headers = '';
    return this._http.get(this.config.apilaravel + "cuestionario/seccion/22", { search: parametros2 }).map(res => {
  ...
  getDatosModeloProf1() {
    ...
    parametros2.set('api_token', api_token);
    let headers = '';
    return this._http.get(this.config.apilaravel + "cuestionario/seccion/21", { search: parametros2 }).map(res => {
      let headers = res.headers;
      let miobjeto = res.json();
      return (miobjeto);
    });
